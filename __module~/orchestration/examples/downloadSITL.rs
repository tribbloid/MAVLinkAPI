...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🚀 Starting arpx runtime to execute download_sitl.yml...");
    
    // Load the YAML profile
    let profile_path = Path::new("download_sitl.yml");
    
    if !profile_path.exists() {
        eprintln!("❌ Error: download_sitl.yml not found in current directory");
